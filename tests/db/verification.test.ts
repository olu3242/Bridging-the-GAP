...
describe("projects", () => {
  it("retains both evidence claims when a competency is independently verified again", async () => {
    ...
    const first = await proveCompetency(l.id, r.id, "brief-ai-concepts");
    const second = await proveCompetency(l.id, r.id, "brief-ai-concepts");
    const rows = await sql<{ id: string; evidence_id: string; superseded_by: string | null }>(
      "select id,evidence_id,superseded_by from verified_skills where profile_id=$1 order by verified_at,id", [l.id]);
    expect(rows).toHaveLength(2);
    const oldClaim = rows.find(row => row.evidence_id === first.evidenceId)!;
    const newClaim = rows.find(row => row.evidence_id === second.evidenceId)!;
