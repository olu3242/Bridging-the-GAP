...
export interface LessonVisualRow {
  ...
  /** Object key in Supabase Storage. Null until the asset has been uploaded. */
  storage_object_path: string | null;
}
export function LessonVisual({ asset, equivalent, slot }: { asset?: LessonVisualRow; equivalent: string; slot: string }) {
  const [failed, setFailed] = useState(false);
  const available = asset?.status === "published" && Boolean(asset.storage_object_path) && !failed;
  return <figure className="my-4 space-y-2 rounded-xl border border-line p-4" aria-label={`${slot} visual`}>
    {available && asset.metadata.format === "webp" ?
      ...
      <p className="text-sm text-ink">This visual is being prepared. The full explanation is in the text below.</p>}
    {available ? <figcaption className="text-sm text-ink">{asset.metadata.caption}</figcaption> : null}
    <details><summary className="cursor-pointer">Text equivalent</summary><p className="mt-2 whitespace-pre-wrap text-sm text-ink">{available ? asset.metadata.extended_description : equivalent}</p></details>
  </figure>;
}
